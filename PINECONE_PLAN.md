...
- Script: `scripts/ingest-pinecone.ts`.
  ...
  - Load and normalize profiles from `data/docs/*.json`.
  ...
  - Upsert to Pinecone with stable ids (e.g., slug of name + gradYear) and store full profile in `metadata`.
  - Optional: sample query to log top matches for verification.
